...
from PyQt5.QtWidgets import QApplication, QMainWindow, QDialog, QLabel, QPushButton, QHBoxLayout, QVBoxLayout, QFrame
...
class AppLogic():
    ...
    def _open_main_window(self) -> None:
        ...
        self.ui_main_window.btn_search.clicked.connect(self._add_search_video)
    ...
    def _generate_default_videos(self) -> None:
        """
        Генерирование видео с выбранных каналов пользователя.
        """

        # Начальное расположение видео.
        self.video_column_num = 0 
        self.video_row_num = 0
        ...
        self._add_content(video_links_and_info)

    def _add_search_video(self) -> None:
        """
        Получение лучшего видео, удовлетворяющего условиям запроса и обновление всех видео в главном экране.
        """
        search_keywords = self.ui_main_window.led_search.text()
        video_links_and_info = self.youtube_parser.search(search_keywords)
        
        self.youtube_parser.get_videos_preview(video_links_and_info)
        self._add_content(video_links_and_info)

    def _add_content(self, video_links_and_info) -> None:
        """Генерация переданных видео."""
        for video_link_and_info in video_links_and_info:
        
            video_layout = self._create_video_layout(video_link_and_info)
            self._add_video_layout(video_layout, self.video_row_num, self.video_column_num)

            self.video_column_num += 1 # Увеличивание столбца видео после каждого добавления.
            self.video_row_num += self.video_column_num // 3 # Увеличивание номера ряда, если номер столбца достиг 3, т.к. в одном ряду 3 элемента
            self.video_column_num %= 3 # Если номер столбца был равен 3, то он обновляется обратно в 0
    
    def _create_video_layout(self, video_link_and_info: list) -> QHBoxLayout:
        """Создание layout видео с ссылкой и информацией о видео.""" 
        ...
        btn_open_video.clicked.connect(lambda: self._open_video(video_link))
    ...
    def _open_video(self, video_page_url: str) -> None:
        ...
        video_url = YouTube(video_page_url).streams.get_by_itag(22).url
